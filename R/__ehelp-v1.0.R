...
ehelp <-function(fun, fn.name=as.character(substitute(fun)), coloring=FALSE, testing=FALSE, output="none") {
#' Enhanced-Help Function: ehelp()
...
#' @export
		...
		for (kwrd in keywords) {
			if (grepl(kwrd,fnLine)) {
				...
				if (keys.count[kwrd] == 0) {
					if (coloring) {
						ehelp.obj <- xcat(crayon::inverse(keys.descrp[kwrd]),ehelp.obj, appendLF=FALSE, testing)
					} else {
						ehelp.obj <- xcat(keys.descrp[kwrd], ehelp.obj, appendLF=FALSE, testing)
					}
				} 
				curLine <- gsub(kwrd,"",fnLine)
				ehelp.obj <- xcat(paste('\t',curLine,'\n'), ehelp.obj, appendLF=FALSE, testing)
	...
	ehelp.obj <- xcat(crayon::bgWhite(keys.descrp["@usage"]), ehelp.obj, appendLF=FALSE, testing)
	ehelp.obj <- xcat(paste0('\t',crayon::blue$bgWhite(paste0(fnName,fn.args)), '\n'),ehelp.obj,,testing)
    } else {
	ehelp.obj <- xcat(keys.descrp["@usage"],ehelp.obj, appendLF=FALSE, testing)
	ehelp.obj <- xcat(paste0('\t',paste0(fnName,fn.args), '\n'),ehelp.obj,,testing)
    }
    ...
    #print(fnArgs)
    #print(keys.count)
	#print.ehelp(ehelp.obj)
	#write(ehelp.obj, file=paste0(fnName,"-eHelp.txt"))
	#print(class(ehelp.obj))

	# process output request...
	processOutput(ehelp.obj, fnName,fnCorpus, output)
	# valid options are "txt", "TXT" or "none"
	# Will ignore anything else and not warn the users of missuse
	if (output=="txt") {
		write.ehelp(ehelp.obj, file=paste0(fnName,"-eHelp.txt"))
	} else if (output == "TXT") {
		write.ehelp(ehelp.obj, file=paste0(fnName,"-eHelp.txt"))
		write.Fncorpus(fnCorpus, file=paste0(fnName,"-eHelp.txt"))
	}

	# preparing for returing output and object...
	class(ehelp.obj) <- "ehelp"
	return(print(ehelp.obj))
}
