...
ehelp(dummyFn)
print( capture.output(cat(paste(capture.output(ehelp(dummyFn)),collpase=''))) )
...
test_that("testing eHelp documentation generated for user-defined functions",{
	output.0 <- capture.output(cat(paste(capture.output(ehelp(nillFn)),collpase='')))
...
# test #1: dummy Fn
test_that("testing eHelp documentation generated for user-defined functions",{
	output.1 <- capture.output(cat(paste(capture.output(ehelp(dummyFnX)),collpase='')))
...
test_that("testing eHelp documentation generated for user-defined functions",{
	output.2 <- capture.output(cat(paste(capture.output(ehelp(dummyFn)),collpase='')))
#	output.2 <- do.call(cat, c(as.list(capture.output(ehelp(dummyFn))),collpase=""))
...
test_that("testing eHelp documentation generated for user-defined functions",{
	output.3 <- utils::capture.output(cat(paste(utils::capture.output(ehelp(myTestFn)),collpase='')))
